...
const Background = () => {
  ...
  class App {
      ...
      ui.textNodes.forEach((el, elIndex) => {
        ...
        for (let i = 0; i < length; i++) {
          ...
          setTimeout(() => {
            ...
            if (i === length - 1 && elIndex === ui.textNodes.length - 1)
              callback();
          }, i * 100);
        }
      });
    ...
    updatePlane() {
      ...
      plane.centerCoords = [
        plane.yCenter * plane.xCell,
        plane.xCenter * plane.yCell,
      ];
    }
    bindNodes() {
    ...
    draw() {
      ...
      if (
        !state.planeIsDrawn ||
        state.dotsProgress < 1 ||
        (state.planeIsDrawn && state.needRedraw)
      ) {
        this.drawPlane();
      }
      ...
      state.animLines.forEach((line, i) => {
        ...
        tools.drawPath(ctx, () => {
          ...
          ctx.fillRect(
            line.x +
              x +
              (isX && p <= 0.5 ? (line.length - length) * line.dir : 0),
            line.y +
              y +
              (isY && p <= 0.5 ? (line.length - length) * line.dir : 0),
            isX ? length : 1,
            isY ? length : 1
          );
        });
    ...
    drawGlitches() {
      ...
      state.glitches.forEach((glitch, i) => {
        ...
        glitch.blinks.forEach((blink) => {
          if (glitch.p >= blink.at && glitch.p <= blink.at + blink.dur)
            show = false;
        });
        if (!show) return;
        tools.drawPath(ctx, () => {
    ...
    drawYMarkup(props) {
      ...
      if (f >= 1) f = 1;
      const f2 =
        conds[0] && conds[1] ? (p - point) / percent : conds[0] ? 1 : 0;
    ...
    drawXMarkup(props) {
      ...
      tools.drawPath(ctx, () => {
        ...
        const textCoords = [
          x + cfg.sectionWidth / 2 + cfg.numberOffset,
          y + cfg.sectionHeight / 2,
        ];
        ctx.fillText(
          xCenter - i2 + "",
    ...
    drawMarkupYAnimation(props) {
      ...
      const f =
        sp >= point && sp <= point + percent ? (sp - point) / percent : 0;
      if (!f) return;
