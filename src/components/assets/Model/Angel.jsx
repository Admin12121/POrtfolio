...
const Angel = React.memo((props) => {
  const group = useRef();
  const { nodes, materials, animations } = useGLTF('/Three/scene.gltf');
  ...
  useEffect(() => {
    const onAnimationLoad = () => {
      if (!initialAnimationPlayed) {
        ...
        }, 1000); // 1000ms delay for the first time
      } else {
        actions.anim.reset().fadeIn(0.5).play();
      }
    };

    // If animations are not loaded yet, wait for them to load
    if (animations.length === 0) {
      console.log("Animation Loading")
    } else {
      onAnimationLoad();
    }
  }, [actions.anim, animations, initialAnimationPlayed]);
